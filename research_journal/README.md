...
**当前方向（最新决定，2026-06）**：把命题从"衰老驱动 IBD"（偏牵强、易被批）**重构为更锋利的问题——「能否把『细胞衰老』从『泛炎症』中因果地剥离出来？IBD 里到底有没有真·衰老细胞？」**。三条并行线对应回答它：
- **task1**（遗传，CPU）：MVMR 调炎症后衰老是否仍因果 + 端粒→IBD + 反向MR + FinnGen 复现。
- **task2**（单细胞 + 基础模型，GPU）⭐**最有新意、优先**：用 Geneformer/scGPT 做迁移学习 + 对 CCL8/CXCR2 做 in-silico 扰动；单卡 3090 即可起步，现有 200G 机器够跑最小版。
- **task3**（转录组，CPU）：多队列留一法验证 + 去卷积扣炎症。
